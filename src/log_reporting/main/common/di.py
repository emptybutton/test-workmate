...
_xyz_report_lines_as_report_views = XyzReportLinesAsReportViews()
...
_multiprocess_xyz_report_parser_from_log_files = (
    MultiprocessReportParserFromLogFiles(
        pool=_process_pool,
        line_separator_parsing_chunk_size=4_000_000,
        divider_for_multiprocess_parsing_of_line_separators=_cpu_count,
...
container.provide(
    GenerateReport(
        report_parser=_multiprocess_xyz_report_parser_from_log_files,
        report_views=_xyz_report_lines_as_report_views,
        report_type=XyzReport,
    ),
    provides=GenerateReport[XyzReport, Path, XyzReportLines],
)
